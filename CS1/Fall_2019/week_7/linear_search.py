...
## Devin Balkcom, heavily modified by THC
## October 2011

# Return the index of key in the_list, or None if key is not present.
def linear_search(the_list, key):
    index = 0                       # start at the beginning of the list
    while index < len(the_list):    # have we seen the entire list?
        if key == the_list[index]:  # have we found key?
            return index            # yes, so index is the answer
        else:
            index += 1              # no, so try the next item
        
    return None                     # we never found key
...
while True:
    ...
    if len(target) == 0:
        break
    index = linear_search(countries, target)
    if index == None:
